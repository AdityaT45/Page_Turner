...
<div class="container">
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                    <tr>
                        ...
                        <td>
                            <!-- Only show the user type as 'User' -->
                            <?php if ($row['user_type'] == 'User') { ?>
                                
                                <a href="admin_manage_users.php?delete=<?php echo $row['id']; ?>" class="action-btn delete-btn" onclick="return confirm('Are you sure you want to delete this user?')">Delete</a>
                            <?php } ?>
                        </td>
                    </tr>
                <?php } ?>
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
            <a href="admin_manage_users.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
        <?php } ?>
    </div>
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $deleteQuery = "DELETE FROM users_info WHERE id = ?";
